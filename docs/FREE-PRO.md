# Free, Pro und Lizenzverwaltung

OfferWeave Free enthält die Basisfunktionen für Angebotskarten, Festpreise, Mengen, alle unterstützten globalen Währungen, Steuereinstellungen, Anfrageformulare und Standardmails. Das separate Pro-Paket ergänzt Staffelpreise, Gruppenberechnung, Auswahlpakete, Varianten, Aktionen, frei platzierbare Eingaben und Banner sowie die vollständige Gestaltung und Mailkonfiguration. Die vollständige Übersicht steht unter **OfferWeave → Free & Pro**.
...
1. Sichern Sie die Konfiguration und die WordPress-Datenbank.
2. Installieren Sie die passende Ausgabe über **Plugins → Plugin hochladen**.
3. Falls bereits eine andere OfferWeave-Ausgabe aktiv ist, deaktivieren Sie diese unter **Plugins**. Aktivieren Sie anschließend die gewünschte Ausgabe und prüfen Sie Ihre Angebotsseiten. Einstellungen und Anfragen bleiben erhalten. Bei einer Netzwerkaktivierung prüfen Sie auch die einzelnen Websites.
4. Verwenden Sie für Pro das öffentliche Pro-Paket und aktivieren Sie die erhaltene Lizenz für die zugehörigen Services.

## English

OfferWeave Free provides offer cards, fixed prices, quantities, all supported global currencies, tax settings, request forms and standard emails. The separate Pro package adds tiered pricing, groups, selection packages, variants, promotions, separately positioned controls and banners, full design controls and email configuration. Compare the editions under **OfferWeave → Free & Pro**.
